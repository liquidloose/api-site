        ...
        <div class="error">        
         <p><?php printf( esc_html__( 'The %s plugin cannot be activated if the following paths were not writable by WordPress', 'prime-mover' ), 
            ...
            <ul>
             ...
             <p><?php echo sprintf(esc_html__('If it is not possible - you can still %s', 'prime-mover' ), '<a class="prime-mover-external-link" target="_blank" href="' . esc_url(CODEXONICS_CUSTOM_CONSTANTS_MU_PLUGINS) . '">' . 
                 esc_html__('add these custom constants to mu-plugins directory.') . '</a>');
                  ?></p>
            </div>
    <?php
    }
